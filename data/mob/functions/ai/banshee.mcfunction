...
tag @e[tag=banshee-rider,sort=nearest,distance=..3] add ai-rider
execute if entity @s[nbt={Passengers:[{Tags:[banshee-rider]}]}] run tag @s add has-rider

#ターゲットのplane-idを記憶
execute as @a[distance=..96] positioned ^ ^ ^180 if entity @s[distance=..200] run tag @s add ai-target-candidate
...
#射撃可能なら射撃
execute as @s[tag=!unattackable,tag=!AngYplus,tag=!AngYminus,tag=!exist-behind,scores={ammunition1=1..,w1-reload=..0}] if entity @p[tag=ai-target,distance=..96] at @s run function mob:weapon/banshee/20mm
...
#particle
execute unless entity @p[tag=ai-target] if entity @s[tag=has-rider] run particle minecraft:dust 0 0.8 1 5 ~ ~ ~ 0.3 0.3 0.3 0 5 force
execute unless entity @p[tag=ai-target] if entity @s[tag=!has-rider] run particle minecraft:dust 0 1 0 5 ~ ~ ~ 0.3 0.3 0.3 0 5 force
execute if entity @p[tag=ai-target] run particle minecraft:dust 1 0 0 5 ~ ~ ~ 0 0 0 1 1 force
...
tag @e[tag=banshee-rider,sort=nearest,distance=..3] remove ai-rider
