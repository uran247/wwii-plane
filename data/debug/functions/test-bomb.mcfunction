...
scoreboard players set @e[tag=test-bomb-init] damage 1250
scoreboard players set @e[tag=test-bomb-init] plane-id 1
execute as @e[tag=test-bomb-init] store result entity @s Attributes[{Name:"generic.knockbackResistance"}].Base double 1 run scoreboard players get @s plane-id
tag @e[tag=test-bomb-init] remove test-bomb-init

#スコア付与
scoreboard players set @e[tag=drop-init,distance=..10] speed 2
scoreboard players set @e[tag=drop-init,distance=..10] age 1200

#角度代入
execute store result entity @e[tag=drop-init,distance=..10,limit=1] Rotation[1] float 0.01 run scoreboard players get #9000 Num
execute store result entity @e[tag=drop-init,distance=..10,limit=1] Rotation[0] float 0.01 run scoreboard players get #9000 Num
