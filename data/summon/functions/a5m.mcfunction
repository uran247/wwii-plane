...
#機体召喚
summon minecraft:armor_stand ~ ~ ~ {Invisible:1,NoGravity:0,Tags:[a5m-root,a5m,plane-init,plane-root,plane,entity-nohit,12p7mm,has-weapon1,tier1,main-weapon1],CustomName:"{\"text\":\"a5m\"}",Pose:{LeftArm:[0f,0f,0f],RightArm:[0f,0f,0f]},DisabledSlots:256,NoGravity:0b}
summon minecraft:armor_stand ~ ~ ~ {Invisible:1,NoGravity:0,Tags:[a5m,plane-init,plane,entity-nohit,plane-seat],CustomName:"{\"text\":\"seat\"}",Pose:{LeftArm:[0f,0f,0f],RightArm:[0f,0f,0f]},Passengers:[{id:donkey,DeathLootTable:"minecraft:entities/bat",SaddleItem:{id:"minecraft:saddle",Count:1b},Tame:1,NoAI:1,Silent:1,ChestedHorse:1b,ActiveEffects:[{Id:14,Amplifier:0,Duration:1000000,ShowParticles:0b}],Tags:[a5m,plane-init,plane,plane-seat,entity-nohit]}],DisabledSlots:256,NoGravity:0b}
...
execute store result score @e[tag=plane-init,tag=a5m-root,limit=1] PosX run data get entity @e[tag=plane-init,tag=a5m-root,limit=1] Pos[0] 100000
execute store result score @e[tag=plane-init,tag=a5m-root,limit=1] PosY run data get entity @e[tag=plane-init,tag=a5m-root,limit=1] Pos[1] 100000
execute store result score @e[tag=plane-init,tag=a5m-root,limit=1] PosZ run data get entity @e[tag=plane-init,tag=a5m-root,limit=1] Pos[2] 100000
...
scoreboard players set @e[tag=engine,tag=plane-init] offsetZ -2800
scoreboard players set @e[tag=aileron-r,tag=plane-init] offsetX -3000
scoreboard players set @e[tag=aileron-r,tag=plane-init] offsetY 500
scoreboard players set @e[tag=aileron-r,tag=plane-init] offsetZ -1500
scoreboard players set @e[tag=aileron-l,tag=plane-init] offsetX 3000
scoreboard players set @e[tag=aileron-l,tag=plane-init] offsetY 500
scoreboard players set @e[tag=aileron-l,tag=plane-init] offsetZ -1500
...
scoreboard players set @e[tag=radder,tag=plane-init] offsetZ 3400
