...
#装備管理
execute as @a[tag=plane-rider,tag=change-inventory] if score #plane-id reg1 = @s[scores={speed=..0}] plane-id run tag @s add check-plane-equip
execute if entity @a[tag=plane-rider,tag=check-plane-equip] at @s[scores={speed=..0}] run function plane:equip/equip-manager
