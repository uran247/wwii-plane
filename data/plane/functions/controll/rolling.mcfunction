...
execute as @e[tag=controll-target,distance=..1] at @s run function plane:controll/rolling/engine-start-sound
...
#スコアを-180 - 180の範囲に収める
execute as @e[tag=controll-target,distance=..1,limit=1] run function plane:controll/flying/correct-ang-y

#ピッチ，ロールを着地時のものに固定
execute as @e[tag=controll-target,distance=..1,limit=1] run function plane:controll/rolling/correct-ang-xz

#speedがtakeoff-speedを超えスロットル全開なら飛行状態に遷移
execute as @e[tag=controll-target,distance=..1,limit=1,scores={throttle=20..}] if score @s takeoff-speed < @s speed run function plane:controll/rolling/takeoff

#speedがpropeller-stopだったら停止モデル、propeller-startだったら滑走モデルに切り替え
execute as @e[tag=controll-target,distance=..1,limit=1] at @s run function plane:controll/rolling/change-plpr-model
