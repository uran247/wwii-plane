...
execute at @s[tag=!has-rider] run tp @e[tag=plane-seat,tag=position-target] ^ ^0.6 ^3

#爆弾装備タグがついていて爆弾が0なら召喚
execute store result score @s[tag=bombed] reg1 if entity @e[tag=position-target,tag=plane-bomb] 
execute as @s[tag=bombed,scores={ammunition2=3,reg1=0}] at @s run function summon:d4y2/d4y2-bomb
execute as @e[tag=d4y2,tag=plane-bomb,distance=..10] if score @s plane-id = #plane-id reg1 run tag @s add position-target
