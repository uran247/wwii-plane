...
#seatの位置修正
execute at @s[tag=has-rider] run tp @e[tag=plane-seat,tag=position-target] ^ ^1 ^-4.2
execute at @s[tag=!has-rider] run tp @e[tag=plane-seat,tag=position-target] ^ ^1.5 ^6.5
