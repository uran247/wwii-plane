...
tag @e[tag=drop-init,distance=..10] remove plane-parts
tag @e[tag=drop-init,distance=..10] remove has-offset
tag @e[tag=drop-init,distance=..10] remove has-model
...
#発射したならreload時間設定
execute if entity @e[tag=drop-init,distance=..20] run scoreboard players set @s w2-reload 2
