...
execute as @s[tag=has-torpedo] if score #offset reg1 matches 1 run summon armor_stand ^ ^-1 ^ {Tags:["g4m1-torpedo",drop-init,torpedo,plane-torpedo,plane-parts,entity-nohit,torpedo,bomb-normal],NoGravity:1b,Invisible:1,HandItems:[{id:"minecraft:diamond_sword",Count:1b,tag:{Damage:48,Unbreakable:1}},{}],Pose:{RightArm:[0f,0f,0f]},DisabledSlots:256}
