...
#n発目の弾なら曳光弾化
scoreboard players operation #is-tracer reg1 = @s ammunition3
scoreboard players operation #is-tracer reg1 %= #5 Num

#召喚
execute unless score #is-tracer reg1 matches 0 run summon minecraft:area_effect_cloud ~ ~ ~ {NoGravity:1,Tags:[projectile,gun,7p7mm,gun-init,tracer-lightblue,entity-nohit,offset-base],Duration:15}
execute if score #is-tracer reg1 matches 1 run summon minecraft:armor_stand ~ ~ ~ {NoGravity:1,Invisible:1,Tags:[projectile,gun,7p7mm,gun-init,tracer-lightblue,entity-nohit,offset-base,tracer],Duration:15}
#tellraw @p [{"score" : {"name":"@s", "objective":"ammunition3"}}, {"text":" "}, {"score" : {"name":"#is-tracer", "objective":"reg1"}}]
execute if entity @e[tag=gun-init,distance=..5,type=armor_stand] run say 1
...
scoreboard players set @e[tag=gun-init,distance=..5] offsetX 2000
